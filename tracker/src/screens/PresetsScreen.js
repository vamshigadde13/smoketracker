...
export function PresetsScreen({ presets, onCreatePreset, onUpdatePreset, onDeletePreset }) {
  ...
  const resetForm = () => {
    setBrand(""); setShortName(""); setQuantity(1); setCostPerSmokeStr(""); setEditingPresetId(null);
  };
  const handleSubmitPreset = async () => {
    if (!brand.trim()) return;
    const payload = { brand, shortName, quantity, costPerSmoke: costPerSmokeStr };
    try {
      setFormError("");
      ...
      setFormError(error?.message || "Unable to save preset.");
    }
  };
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-gray-50">
      <View className="flex-1 p-5">
        ...
        <View className="mb-5 rounded-2xl bg-white p-4 shadow-sm">
          ...
          <Text className="mb-1 text-sm font-medium text-gray-600">Short name (optional)</Text>
          <TextInput value={shortName} onChangeText={setShortName} placeholder="e.g. ADV" className="mb-3 rounded-xl border border-gray-200 px-3 py-3" />
          ...
          <Text className="mb-1 text-sm font-medium text-gray-600">Cost per smoke</Text>
          ...
          {formError ? <Text className="mb-3 text-sm font-medium text-red-600">{formError}</Text> : null}
          ...
          renderItem={({ item }) => (
            <View className="mb-2 rounded-xl bg-white px-4 py-3 shadow-sm">
              ...
              <View className="mt-2 flex-row">
                <Pressable onPress={() => { setEditingPresetId(item.id); setBrand(item.brand); setShortName(item.shortName || ""); setQuantity(item.quantity); setCostPerSmokeStr(item.costPerSmoke != null ? String(item.costPerSmoke) : ""); }} className="mr-2 rounded-lg bg-gray-100 px-3 py-2"><Text className="text-sm font-semibold text-gray-700">Edit</Text></Pressable>
